...
// Component for messages that are only emojis (large display)
export const EmojiOnlyMessage = ({ text, className = '' }) => {
  ...
  const parts = parseEmojiText(text);
  const isEmojiOnly = parts.every(part => 
  ...
  const sizeClass = emojiCount <= 3 ? 'text-4xl' : emojiCount <= 6 ? 'text-3xl' : 'text-2xl';
      ...
      {parts.map((part, index) => {
        if (part.type === 'emoji') {
          return (
            <span 
              key={index} 
              className={`${sizeClass} inline-block mx-1`}
            >
              {part.content}
            </span>
          );
...
export const QuickReactions = ({ onEmojiClick, disabled = false }) => {
  ...
  return (
    <div className="flex gap-1 p-2 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700 mb-2">
      <span className="text-xs text-slate-500 dark:text-slate-400 mr-2 self-center">Nhanh:</span>
